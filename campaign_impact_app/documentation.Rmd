...
- `app.R`: script that loads libraries requiered for the analysis, data and the functions.
...
- `fun_adstock_calc`: adstock function 
    
    $$adstock_{(i)} = investment_{(i)} + [ \alpha \cdot adstock_{(i-1)} ]$$
...
- `fun_model_lm`: model 1 function

$$Search\;Volume_{t} = \beta_0 + \beta_1 * Adstock_{t} + \sum_{i=1}^3 \beta_{2,i }*campaign_i$$
...
- `fun_model_bayes`: model 2 function

$$Search\;Volume_{t} = \beta_0 + Search\;Volume_{t-1} + \beta_1 * Adstock_{t} + \sum_{i=1}^3 \beta_{2,i }*campaign_i$$
